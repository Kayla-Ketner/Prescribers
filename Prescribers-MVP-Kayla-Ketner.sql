...
--1a:
SELECT npi, SUM(total_claim_count) AS claim_total
FROM prescription 
GROUP BY npi
...
--1b:
SELECT npi, nppes_provider_first_name, nppes_provider_last_org_name, specialty_description, SUM(total_claim_count) AS claim_total
...
--2d:difficult
SELECT specialty_description, ROUND((MAX(opioid_claims)/(MAX(total_claims))*100),2) AS percentage_opioid
FROM (SELECT specialty_description, SUM(total_claim_count) AS opioid_claims
	FROM prescriber INNER JOIN prescription USING(npi) INNER JOIN drug USING (drug_name)
	WHERE opioid_drug_flag='Y'
...
SELECT generic_name, 										ROUND(SUM(total_drug_cost)/SUM(total_day_supply),2)::money AS max_per_day
FROM drug INNER JOIN prescription USING(drug_name)
GROUP BY generic_name
...
--Another Solution(DISTINCT makes difference)
...
FROM drug_type_cost	INNER JOIN prescription USING (drug_name)
WHERE drug_type = 'opioid' OR drug_type = 'antibiotic'
GROUP BY drug_type;
...
--5b: largest pop=Nashville-Davidson-Murfreesboro-Franklin, TN
